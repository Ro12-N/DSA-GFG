...
// TC: O((n + q) * log n) where n is the size of the input array and q is the number of queries (due to segment tree build and query/update operations) 
// SC: O(n) for the segment tree and O(q) for the answer vector, resulting in O(n + q) overall space complexity